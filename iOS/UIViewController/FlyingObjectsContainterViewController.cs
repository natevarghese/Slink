...
namespace Slink.iOS {
    public partial class FlyingObjectsContainterViewController : UIViewController
    {
        ...
        nfloat AddViewSuperviewHeight = 50;          string AdKeyGender = "gender";          public ClearNavigationController ContainerNavigationController = new ClearNavigationController();
        ...
        public override void ViewWillAppear(bool animated)
        {
            ...
            });              AdViewSuperViewHeightConstaint.Constant = ShowsAds ? AddViewSuperviewHeight : 0;             if (ShowsAds)             {                 var iPersistant = ServiceLocator.Instance.Resolve<IPersistantStorage>();                 var facebookToken = iPersistant.GetFacebookToken();                                      var graphRequest = new GraphRequest("/me?fields=gender,birthday", null, facebookToken, null, "GET");                 var requestConnection = new GraphRequestConnection();                 requestConnection.AddRequest(graphRequest, (connection, result, error) =>                 {                     var data = result as NSDictionary;                     if (data == null) return;                      var dict = new Dictionary<string, string>();                      if(data.ContainsKey(new NSString(AdKeyGender)))                         dict.Add(AdKeyGender, data[AdKeyGender].ToString());                     ShowBanner(dict);                 });                 requestConnection.Start();             }
